...
let { ownerNumber, groupLimit, memberLimit, prefix } = setting;
...
module.exports = HandleMsg = async (aruga, message) => {
  try {
    ...
    switch (command) {
      ...
      case "help":
        await aruga
          .sendText(from, menuId.textMenu(pushname))
          .then(() =>
      ...
      case "ownerbot":
        await aruga
          ...
          .then(() =>
            aruga.sendText(
              from,
              "Jika kalian ingin request fitur silahkan chat nomor owner!"
            )
          );
        break;
      case "join":
        ...
        if (isOwnerBot) {
          await aruga.joinGroupViaLink(linkgrup).then(async () => {
            ...
            await aruga.sendText(
              chekgrup.id,
              `Hai minna~, Im Aruga BOT. To find out the commands on this bot type ${prefix}menu`
            );
          });
        } else {
      ...
      case "stikergif":
        if (isMedia || isQuotedVideo) {
          if (
            ...
            await exec(
              `gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`,
            ...
            aruga.reply(
              from,
              `[❗] Kirim gif dengan caption *${prefix}stickergif* max 10 sec!`,
              id
            );
          }
        } else {
          aruga.reply(
            from,
            `[❗] Kirim gif dengan caption *${prefix}stickergif*`,
            id
          );
        }
        break;
      ...
      case "meme":
        if ((isMedia || isQuotedImage) && args.length >= 2) {
          ...
          aruga
            .sendFile(from, ImageBase64, "image.png", "", null, true)
            .then(() => {
      ...
      case "nulis":
        ...
        await aruga.sendImage(from, `${nulisp}`, "", "Nih...", id).catch(() => {
          aruga.reply(from, "Ada yang Error!", id);
        });
        break;
        ...
        var idx = data.findIndex(function (post, index) {
          if (
            post.name.transliteration.id.toLowerCase() ==
              args[0].toLowerCase() ||
            post.name.transliteration.en.toLowerCase() == args[0].toLowerCase()
          )
            return true;
        });
        var pesan = "";
      ...
      case "ytmp3":
        ...
        const linkmp3 = args[0]
          .replace("https://youtu.be/", "")
          .replace("https://www.youtube.com/watch?v=", "");
        rugaapi.ytmp3(`https://youtu.be/${linkmp3}`).then(async (res) => {
          if (res.error)
            return aruga.sendFileFromUrl(
              from,
              `${res.url}`,
              "",
              `${res.error}`
          ...
          await aruga
            ...
            .catch(() => {
              aruga.reply(
                from,
                `URL INI ${args[0]} SUDAH PERNAH DI DOWNLOAD SEBELUMNYA ..URL AKAN RESET SETELAH 60 MENIT`,
                id
              );
            });
        });
      ...
      case "ytmp4":
        ...
        const linkmp4 = args[0]
          .replace("https://youtu.be/", "")
          .replace("https://www.youtube.com/watch?v=", "");
        rugaapi.ytmp4(`https://youtu.be/${linkmp4}`).then(async (res) => {
          if (res.error)
            return aruga.sendFileFromUrl(
              from,
              `${res.url}`,
      ...
      case "facebook":
        if (args.length == 0)
          return aruga.reply(
            from,
            `Untuk mendownload video dari link facebook\nketik: ${prefix}fb [link_fb]`,
            id
          );
        rugaapi.fb(args[0]).then(async (res) => {
          ...
          if (res.status == "error")
            return aruga.sendFileFromUrl(
              ...
              link,
              "",
              "Maaf url anda tidak dapat ditemukan",
              id
            );
          await aruga
            .sendFileFromUrl(from, linkhd, "", "Nih ngab videonya", id)
            .catch(async () => {
              await aruga
                .sendFileFromUrl(from, linksd, "", "Nih ngab videonya", id)
                .catch(() => {
      ...
      case "memes":
        ...
        aruga.sendFileFromUrl(from, randmeme, "", "", id).catch(() => {
          aruga.reply(from, "Ada yang Error!", id);
        });
        break;
      ...
      case "sreddit":
        ...
        await aruga.sendFileFromUrl(from, hasilreddit, "", "", id).catch(() => {
          aruga.reply(from, "Ada yang Error!", id);
        });
        break;
      case "resep":
      ...
      case "nekopoi":
        rugapoi
          .getLatest()
          .then((result) => {
            rugapoi.getVideo(result.link).then((res) => {
              let heheq = "\n";
      ...
      case "stalkig":
        ...
        await aruga
          .sendFileFromUrl(from, igstalkpict, "", igstalk, id)
          .catch(() => {
            aruga.reply(from, "Ada yang Error!", id);
          });
        break;
      case "wiki":
        ...
        await aruga.reply(from, wikis, id).catch(() => {
          aruga.reply(from, "Ada yang Error!", id);
        });
        break;
      case "cuaca":
        ...
        await aruga.reply(from, cuacap, id).catch(() => {
          aruga.reply(from, "Ada yang Error!", id);
        });
        break;
      case "lyrics":
      ...
      case "chord":
        ...
        await aruga.reply(from, chordp, id).catch(() => {
          aruga.reply(from, "Ada yang Error!", id);
        });
        break;
      ...
      case "play": //silahkan kalian custom sendiri jika ada yang ingin diubah
        ...
        axios
          .get(`https://arugaytdl.herokuapp.com/search?q=${body.slice(6)}`)
          .then(async (res) => {
            await aruga.sendFileFromUrl(
              from,
            ...
            rugaapi
              ...
              .then(async (res) => {
                ...
                await aruga
                  .sendFileFromUrl(from, `${res.link}`, "", "", id)
                  .catch(() => {
                    aruga.reply(
                      from,
                      `URL INI ${args[0]} SUDAH PERNAH DI DOWNLOAD SEBELUMNYA ..URL AKAN RESET SETELAH 60 MENIT`,
      ...
      case "movie":
        if (args.length == 0)
          return aruga.reply(
            from,
            `Untuk mencari suatu movie dari website sdmovie.fun\nketik: ${prefix}movie judulnya`,
            id
          );
        rugaapi.movie(body.slice(7)).then(async (res) => {
      ...
      case "shortlink":
        ...
        await aruga.sendText(from, shortlink).catch(() => {
          aruga.reply(from, "Ada yang Error!", id);
        });
        break;
      case "bapakfont":
      ...
      case "simisimi":
        ...
        aruga.reply(
          from,
          `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`,
          id
        );
        break;
      case "simi":
      ...
      case "bc": //untuk broadcast atau promosi
        ...
        for (let idk of chatz) {
          ...
          if (!cvk.isReadOnly)
            aruga.sendText(idk, `〘 *Whatsapp* 〙\n\n${msg}`);
          if (cvk.isReadOnly) aruga.sendText(idk, `〘 *Whatsapp* 〙\n\n${msg}`);
        }
      ...
      case "leaveall": //mengeluarkan bot dari semua group serta menghapus chatnya
        ...
        for (let gclist of allGroupz) {
          await aruga.sendText(
            gclist.contact.id,
            `Maaf bot sedang pembersihan, total chat aktif : ${allChatz.length}`
          );
    ...
    if (!isCmd && isGroupMsg && isNgegas) {
      ...
      if (find && find.id === groupId) {
        ...
        if (cekuser && isIn !== false) {
          ...
          if (cekMember.length === 0) {
            if (isKasar) {
              db.get("group")
                .find({ id: groupId })
                .set("members", [{ id: pengirim, denda: 5000 }])
                .write();
            } else {
              db.get("group")
                .find({ id: groupId })
        ...
        if (isKasar) {
          db.get("group")
            .push({ id: groupId, members: [{ id: pengirim, denda: 5000 }] })
            .write();
          await aruga.reply(
            from,
            "Jangan badword bodoh\nDenda +5.000\nTotal : Rp5.000",
